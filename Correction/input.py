...
class InputRPACC(object):
    ...
    def get_cluster(self):
        ...
        with open(json_path, 'r') as f:
            data = json.load(f)
        cluster_data = data['cluster']['cutted cluster']
        cluster = []
        for a in cluster_data:
            atom = Atom(a['nat'], a['x'], a['y'], a['z'], no=a['no'], atom_type=a['type'], coor=a['coordinate number'], coor_vec=a['coordinate vector'])
            atom.layer = a['layer']
            cluster.append(atom)
        return cluster
